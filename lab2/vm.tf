...
resource "azurerm_public_ip" "spoke_vm" {
  for_each            = { for key, value in local.spoke_subnets : key => value if value.name == "SubnetA" && var.environment_name == "dev" }
  name                = "${lower(each.value.spoke_key)}pip-${var.application_name}-${var.environment_name}-spoke-vm"
  resource_group_name = azurerm_resource_group.this.name
  location            = azurerm_resource_group.this.location
...
resource "azurerm_network_interface" "spoke_nicA" {
  for_each            = { for key, value in local.spoke_subnets : key => value if value.name == "SubnetA" && var.environment_name == "dev" }
  name                = "${lower(each.value.spoke_key)}nicA-${var.application_name}-${var.environment_name}"
  location            = azurerm_resource_group.this.location
  resource_group_name = azurerm_resource_group.this.name
  ...
  ip_configuration {
    ...
    private_ip_address_allocation = "Dynamic"
    public_ip_address_id          = azurerm_public_ip.spoke_vm[each.key].id
  }
}
...
resource "azurerm_network_interface" "spoke_nicB" {
  ...
  name                = "${lower(each.value.spoke_key)}nicB-${var.application_name}-${var.environment_name}"
  location            = azurerm_resource_group.this.location
  resource_group_name = azurerm_resource_group.this.name
...
resource "azurerm_linux_virtual_machine" "spoke_vmA" {
  for_each            = { for key, value in local.spoke_subnets : key => value if value.name == "SubnetA" && var.environment_name == "dev" }
  name                = "${lower(each.value.spoke_key)}vmA${var.application_name}-${var.environment_name}"
  resource_group_name = azurerm_resource_group.this.name
  location            = azurerm_resource_group.this.location
  size                = "Standard_B2s"
  admin_username      = "adminuser"
  network_interface_ids = [
    ...
    azurerm_network_interface.spoke_nicA[each.key].id
  ]
  identity {
    type = "SystemAssigned"
  }
...
resource "azurerm_linux_virtual_machine" "spoke_vmB" {
  for_each            = { for key, value in local.spoke_subnets : key => value if value.name == "SubnetB" && var.environment_name == "dev" }
  name                = "${lower(each.value.spoke_key)}vmB${var.application_name}-${var.environment_name}"
  resource_group_name = azurerm_resource_group.this.name
  location            = azurerm_resource_group.this.location
  size                = "Standard_B2s"
  admin_username      = "adminuser"
  network_interface_ids = [
    ...
    azurerm_network_interface.spoke_nicB[each.key].id
  ]
  identity {
    type = "SystemAssigned"
  }
